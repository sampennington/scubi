...
import {
  ...
  extractFontFamiliesAndSources,
  isNonSystemFont,
  pickPalette,
  scoreColorsFromCss
} from "@/scraper/utils/css"
import {
  extractSeoMeta,
  extractPlainText,
  extractImages as extractPageImages
...
async function extractColorsAndFonts(homeUrl: string): Promise<{
  colors: ReturnType<typeof ColorPaletteSchema.parse>
  fonts: ReturnType<typeof FontsSchema.parse>
  renderCssSample: string[]
  html: string
}> {
  const renderer = new PlaywrightRenderer()
  const rendered = await renderer.render(homeUrl)
  const $home = cheerio.load(rendered.html)

  const cssUrls = [
    ...new Set([...collectStylesheetUrls($home, homeUrl, normalizeUrl), ...rendered.cssUrls])
  ]
  const cssTexts = (await Promise.all(cssUrls.map(fetchText))).filter((t): t is string => !!t)
  const renderCssSample = cssTexts.slice(0, 3)

  const colorOrder = scoreColorsFromCss(cssTexts)
  const colors = pickPalette(colorOrder)

  const { families, sources } = extractFontFamiliesAndSources($home, cssTexts)
  const ordered = families.filter((f) => isNonSystemFont(f))
  const fonts = FontsSchema.parse({
    heading: ordered[0],
  ...
  return { colors, fonts, renderCssSample, html: rendered.html }
}
...
export async function scrapeSite(targetUrl: Url): Promise<SiteScrape> {
  ...
  try {
    ...
    const {
      colors: c,
      fonts: f,
      renderCssSample: sample,
      html
    } = await extractColorsAndFonts(homeUrl)
    colors = c
    fonts = f
    renderCssSample = sample
